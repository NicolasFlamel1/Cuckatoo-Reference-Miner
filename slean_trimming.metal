...
Trimming round 4: clear number of edges per bucket one, step thirteen, step fourteen, clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, step fifteen, ..., clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step sixteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, ..., clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen
...
Trimming round n - 1: clear number of edges per bucket one, step thirteen, step fourteen, clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, step fifteen, ..., clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step sixteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, ..., clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen
Trimming round n: clear number of edges per bucket one, step thirteen, step fourteen, clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, step fifteen, ..., clear number of edges per bucket one, step thirteen, step fifteen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step sixteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen, ..., clear number of edges per bucket one, step thirteen, clear number of edges per bucket two, step eighteen, step seventeen
...
// Check if local buckets size is one
	...
	// Check if initial buckets isn't disjointed
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId);
	
	// Otherwise
	#else
	
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, device uint *bucketsSecondPart, const uint globalId);
	#endif
...
// Otherwise check if local buckets size is two
	...
	// Check if initial buckets isn't disjointed
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint2 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId, const ushort localId, const ushort localSize);
	
	// Otherwise
	#else
	
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint2 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, device uint2 *bucketsSecondPart, const uint globalId, const ushort localId, const ushort localSize);
	#endif
...
// Otherwise
	...
	// Check if initial buckets isn't disjointed
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint4 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId, const ushort localId, const ushort localSize);
	
	// Otherwise
	#else
	
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint4 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, device uint4 *bucketsSecondPart, const uint globalId, const ushort localId, const ushort localSize);
	#endif
#endif
...
// Check if local buckets size is one
	...
	[[kernel]] void trimEdgesStepSeven(device const ulong *edgesBitmap, device uint *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId);

// Otherwise check if local buckets size is two
#elif LOCAL_BUCKETS_SIZE == 2

	// Trim edges step seven
	[[kernel]] void trimEdgesStepSeven(device const ulong *edgesBitmap, device uint2 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId, const ushort localId, const ushort localSize);

// Otherwise
#else

	// Trim edges step seven
	[[kernel]] void trimEdgesStepSeven(device const ulong *edgesBitmap, device uint4 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const ulong4 &sipHashKeys, const uint globalId, const ushort localId, const ushort localSize);
#endif
...
[[kernel]] void trimEdgesStepThirteen(device const ulong *edgesBitmap, device uint2 *buckets, device atomic_uint *numberOfEdgesPerBucket, constant const uchar &part, constant const uchar &nodesInSecondPartition, constant const ulong4 &sipHashKeys, const uint globalId);

// Trim edges step fourteen
[[kernel]] void trimEdgesStepFourteen(device const uint2 *buckets, device const uint *numberOfEdgesPerBucket, device uint *nodesBitmap, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step fifteen
[[kernel]] void trimEdgesStepFifteen(device const uint2 *buckets, device const uint *numberOfEdgesPerBucket, device uint *nodesBitmap, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step sixteen
[[kernel]] void trimEdgesStepSixteen(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *nodesBitmap, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, const ushort localId, const ushort localSize, const uint groupId);
...
[[kernel]] void trimEdgesStepEighteen(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device const uint *nodesBitmap, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const uchar &part, const ushort localId, const ushort localSize, const uint groupId);
...
// Check if number of remaining edges buckets is one
	...
	// Clear number of edges per destination bucket
	[[kernel]] void clearNumberOfEdgesPerDestinationBucket(device uint *numberOfEdgesPerDestinationBucket, const uint globalId);
	
// Otherwise
#else

	// Clear number of edges per destination bucket
	[[kernel]] void clearNumberOfEdgesPerDestinationBucket(device ulong *numberOfEdgesPerDestinationBucket, const uint globalId);
#endif
...
// Check if local buckets size is one
	...
	// Check if initial buckets isn't disjointed
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], const uint globalId [[thread_position_in_grid]]) {
	
	// Otherwise
	#else
	
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], device uint *bucketsSecondPart [[buffer(4)]], const uint globalId [[thread_position_in_grid]]) {
	#endif
...
// Otherwise check if local buckets size is two
	...
	// Check if initial buckets isn't disjointed
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint2 *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], const uint globalId [[thread_position_in_grid]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]]) {
	
	// Otherwise
	#else
	
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint2 *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], device uint2 *bucketsSecondPart [[buffer(4)]], const uint globalId [[thread_position_in_grid]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]]) {
	#endif
...
// Otherwise
	...
	// Otherwise
		...
		// Trim edges step one
		[[kernel]] void trimEdgesStepOne(device uint4 *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], device uint4 *bucketsSecondPart [[buffer(4)]], const uint globalId [[thread_position_in_grid]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]]) {
	#endif
#endif
...
// Otherwise
	...
	// Check if initial buckets isn't disjointed
		...
		device const uint *indices = &buckets[static_cast<ulong>(INITIAL_MAX_NUMBER_OF_EDGES_PER_BUCKET) * groupId];
	
	// Otherwise
	#else
	
		// Get work group's bucket's indices
		device const uint *indices = (groupId < INITIAL_BUCKETS_NUMBER_OF_BUCKETS) ? &buckets[static_cast<ulong>(INITIAL_MAX_NUMBER_OF_EDGES_PER_BUCKET) * groupId] : &bucketsSecondPart[static_cast<ulong>(INITIAL_MAX_NUMBER_OF_EDGES_PER_BUCKET) * (groupId - INITIAL_BUCKETS_NUMBER_OF_BUCKETS)];
	#endif
...
// Otherwise
	...
	// Trim edges step four
	[[kernel]] void trimEdgesStepFour(device const uint *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *nodesBitmap [[buffer(5)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(3)]], device const uint *sourceBucketsSecondPart [[buffer(4)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
#endif
...
// Trim edges step five
[[kernel]] void trimEdgesStepFive(device const uint *buckets [[buffer(6)]], device const uint *numberOfEdgesPerBucket [[buffer(7)]], device uint *edgesBitmap [[buffer(8)]], constant const uchar &part [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	device uint *edgesBitmapIndex = &edgesBitmap[groupId * static_cast<short>(NUMBER_OF_REMAINING_EDGES_BITMAP_BYTES / sizeof(uint)) + part * static_cast<int>(((NUMBER_OF_EDGES / BITS_IN_A_BYTE) / SLEAN_TRIMMING_PARTS) / sizeof(uint))];
...
// Check if initial buckets isn't disjointed
	...
	// Trim edges step six
	[[kernel]] void trimEdgesStepSix(device const uint *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device const uint *nodesBitmap [[buffer(5)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
...
// Check if local buckets size is one
	...
	// Trim edges step seven
	[[kernel]] void trimEdgesStepSeven(device const ulong *edgesBitmap [[buffer(8)]], device uint *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const ulong4 &sipHashKeys [[buffer(3)]], const uint globalId [[thread_position_in_grid]]) {
...
// Otherwise
	...
	// Check if local buckets size is one
		...
		// Get work item's edge indices in the part
		const uint indices = globalId * static_cast<char>(sizeof(ulong) * BITS_IN_A_BYTE) + part * (NUMBER_OF_EDGES / SLEAN_TRIMMING_PARTS);
		
		// Get this work item's edges in the part
		ulong edges = edgesBitmap[globalId + part * static_cast<int>(((NUMBER_OF_EDGES / BITS_IN_A_BYTE) / SLEAN_TRIMMING_PARTS) / sizeof(ulong))];
...
// Trim edges step thirteen
[[kernel]] void trimEdgesStepThirteen(device const ulong *edgesBitmap [[buffer(8)]], device uint2 *buckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerBucket [[buffer(1)]], constant const uchar &part [[buffer(2)]], constant const uchar &nodesInSecondPartition [[buffer(9)]], constant const ulong4 &sipHashKeys [[buffer(3)]], const uint globalId [[thread_position_in_grid]]) {
...
// Trim edges step fourteen
[[kernel]] void trimEdgesStepFourteen(device const uint2 *buckets [[buffer(0)]], device const uint *numberOfEdgesPerBucket [[buffer(1)]], device uint *nodesBitmap [[buffer(5)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
...
// Trim edges step fifteen
[[kernel]] void trimEdgesStepFifteen(device const uint2 *buckets [[buffer(0)]], device const uint *numberOfEdgesPerBucket [[buffer(1)]], device uint *nodesBitmap [[buffer(5)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
...
// Trim edges step sixteen
[[kernel]] void trimEdgesStepSixteen(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *nodesBitmap [[buffer(5)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		if(!isBitSetInBitmap(bitmap, edgeIndexAndNode.y ^ 1)) {
			...
			// Get edge's index's bucket index
			const uint bucketIndex = ((edgeIndexAndNode.x - (NUMBER_OF_EDGES / SLEAN_TRIMMING_PARTS) * (SLEAN_TRIMMING_PARTS - 1)) * SLEAN_TRIMMING_PARTS) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_REMAINING_EDGES_BUCKET_SORTING;
...
// Trim edges step eighteen
[[kernel]] void trimEdgesStepEighteen(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device const uint *nodesBitmap [[buffer(5)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const uchar &part [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		if(!isBitSetInBitmap(bitmap, edgeIndexAndNode.y ^ 1)) {
			...
			// Get edge's index's bucket index
			const uint bucketIndex = ((edgeIndexAndNode.x - (NUMBER_OF_EDGES / SLEAN_TRIMMING_PARTS) * part) * SLEAN_TRIMMING_PARTS) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_REMAINING_EDGES_BUCKET_SORTING;
...
// Check if number of remaining edges buckets is one
	...
	// Clear number of edges per destination bucket
	[[kernel]] void clearNumberOfEdgesPerDestinationBucket(device uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], const uint globalId [[thread_position_in_grid]]) {
	
// Otherwise
#else

	// Clear number of edges per destination bucket
	[[kernel]] void clearNumberOfEdgesPerDestinationBucket(device ulong *numberOfEdgesPerDestinationBucket [[buffer(7)]], const uint globalId [[thread_position_in_grid]]) {
#endif
