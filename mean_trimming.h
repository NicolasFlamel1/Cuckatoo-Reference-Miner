...
// Mean trimming number of bits for buckets
#define MEAN_TRIMMING_NUMBER_OF_BITS_FOR_BUCKETS max(EDGE_BITS - static_cast<int>(bit_width(static_cast<unsigned int>(MEAN_TRIMMING_TARGET_WORK_GROUP_RAM_BYTES * BITS_IN_A_BYTE))) + 1, 2)

// Mean trimming number of buckets
#define MEAN_TRIMMING_NUMBER_OF_BUCKETS (1 << MEAN_TRIMMING_NUMBER_OF_BITS_FOR_BUCKETS)

// Check if using macOS
#ifdef __APPLE__

	// Mean trimming number of most significant bits used for bucket sorting
	#define MEAN_TRIMMING_NUMBER_OF_MOST_SIGNIFICANT_BITS_USED_FOR_BUCKET_SORTING (sizeof(uint32_t) * BITS_IN_A_BYTE - EDGE_BITS + MEAN_TRIMMING_NUMBER_OF_BITS_FOR_BUCKETS)
	
	// Mean trimming number of least significant bits ignored during bucket sorting
	#define MEAN_TRIMMING_NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_BUCKET_SORTING (sizeof(uint32_t) * BITS_IN_A_BYTE - MEAN_TRIMMING_NUMBER_OF_MOST_SIGNIFICANT_BITS_USED_FOR_BUCKET_SORTING)
	
// Otherwise
#else

	// Mean trimming number of most significant bits used for bucket sorting
	#define MEAN_TRIMMING_NUMBER_OF_MOST_SIGNIFICANT_BITS_USED_FOR_BUCKET_SORTING (sizeof(cl_uint) * BITS_IN_A_BYTE - EDGE_BITS + MEAN_TRIMMING_NUMBER_OF_BITS_FOR_BUCKETS)
	
	// Mean trimming number of least significant bits ignored during bucket sorting
	#define MEAN_TRIMMING_NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_BUCKET_SORTING (sizeof(cl_uint) * BITS_IN_A_BYTE - MEAN_TRIMMING_NUMBER_OF_MOST_SIGNIFICANT_BITS_USED_FOR_BUCKET_SORTING)
#endif

// Mean trimming number of items per bucket
#define MEAN_TRIMMING_NUMBER_OF_ITEMS_PER_BUCKET (1 << MEAN_TRIMMING_NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_BUCKET_SORTING)

// Mean trimming bucket item mask
#define MEAN_TRIMMING_BUCKET_ITEM_MASK (MEAN_TRIMMING_NUMBER_OF_ITEMS_PER_BUCKET - 1)

// Mean trimming initial max number of edges per bucket
...
// Mean trimming after trimming round max number of edges per bucket
...
#define MEAN_TRIMMING_NUMBER_OF_BITMAP_BYTES (MEAN_TRIMMING_NUMBER_OF_ITEMS_PER_BUCKET / BITS_IN_A_BYTE)
...
// Check if using macOS
	...
	static inline MTL::Device *createMeanTrimmingContext() noexcept;
...
// Otherwise
	...
	// Create mean trimming context
	static inline cl_context createMeanTrimmingContext(const cl_platform_id platforms[], const cl_uint numberOfPlatforms) noexcept;
...
// Check if using macOS
	...
	// Create mean trimming context
	MTL::Device *createMeanTrimmingContext() noexcept {
		...
		if(devices) {
			...
			for(NS::UInteger i = 0; i < devices->count(); ++i) {
				...
				// Check if device supports the Metal version, its memory size is large enough, and its work group memory size is large enough
				if(device && device->supportsFamily(MTL::GPUFamilyMetal3) && device->recommendedMaxWorkingSetSize() >= MEAN_TRIMMING_REQUIRED_RAM_BYTES && device->maxThreadgroupMemoryLength() >= MEAN_TRIMMING_REQUIRED_WORK_GROUP_RAM_BYTES) {
...
// Otherwise
	...
	// Create mean trimming context
	cl_context createMeanTrimmingContext(const cl_platform_id platforms[], const cl_uint numberOfPlatforms) noexcept {
...
// Check if using macOS
	...
	bool performMeanTrimmingLoop(MTL::Device *device) noexcept {
		...
		const unsigned int localBucketsSize = min(bit_floor((device->maxThreadgroupMemoryLength() - MEAN_TRIMMING_NUMBER_OF_BUCKETS) / (sizeof(uint32_t) * MEAN_TRIMMING_NUMBER_OF_BUCKETS) + 1), static_cast<NS::UInteger>(MEAN_TRIMMING_MAX_LOCAL_BUCKETS_SIZE));
		...
		static const unique_ptr<NS::Dictionary, void(*)(NS::Dictionary *)> preprocessorMacros(NS::Dictionary::alloc()->init((const NS::Object *[]){
			...
			// Number of bits for buckets value
			unique_ptr<NS::Number, void(*)(NS::Number *)>(NS::Number::alloc()->init(MEAN_TRIMMING_NUMBER_OF_BITS_FOR_BUCKETS), [](NS::Number *numberOfBitsForBucketsValue) noexcept {
			...
			// Number of bits for buckets key
			MTLSTR("NUMBER_OF_BITS_FOR_BUCKETS"),
		...
		}, 10), [](NS::Dictionary *preprocessorMacros) noexcept {
		...
		static const unique_ptr<MTL::Function, void(*)(MTL::Function *)> clearNumberOfEdgesPerBucketKernel(library->newFunction(MTLSTR("clearNumberOfEdgesPerBucket")), [](MTL::Function *clearNumberOfEdgesPerBucketKernel) noexcept {
		...
		static unique_ptr<MTL::ComputePipelineState, void(*)(MTL::ComputePipelineState *)> clearNumberOfEdgesPerBucketPipeline(device->newComputePipelineState(clearNumberOfEdgesPerBucketKernel.get(), &createPipelineClearNumberOfEdgesPerBucketError), [](MTL::ComputePipelineState *clearNumberOfEdgesPerBucketPipeline) noexcept {
		...
		static const unique_ptr<MTL::ComputePipelineDescriptor, void(*)(MTL::ComputePipelineDescriptor *)> clearNumberOfEdgesPerBucketPipelineDescriptor(MTL::ComputePipelineDescriptor::alloc()->init(), [](MTL::ComputePipelineDescriptor *clearNumberOfEdgesPerBucketPipelineDescriptor) noexcept {
		...
		static const unique_ptr<MTL::Buffer, void(*)(MTL::Buffer *)> bucketsOne(device->newBuffer(static_cast<uint64_t>(MEAN_TRIMMING_INITIAL_MAX_NUMBER_OF_EDGES_PER_BUCKET) * bucketsOneNumberOfBuckets * sizeof(uint32_t), MTL::ResourceStorageModePrivate | MTL::ResourceHazardTrackingModeUntracked), [](MTL::Buffer *bucketsOne) noexcept {
		...
		static const unique_ptr<MTL::Buffer, void(*)(MTL::Buffer *)> numberOfEdgesPerBucketOne(device->newBuffer(MEAN_TRIMMING_NUMBER_OF_BUCKETS * sizeof(uint32_t), MTL::ResourceStorageModePrivate | MTL::ResourceHazardTrackingModeUntracked), [](MTL::Buffer *numberOfEdgesPerBucketOne) noexcept {
		
			// Free number of edges per bucket one
			numberOfEdgesPerBucketOne->release();
		});
		static const unique_ptr<MTL::Buffer, void(*)(MTL::Buffer *)> numberOfEdgesPerBucketTwo(device->newBuffer(MEAN_TRIMMING_NUMBER_OF_BUCKETS * sizeof(uint32_t), MTL::ResourceStorageModePrivate | MTL::ResourceHazardTrackingModeUntracked), [](MTL::Buffer *numberOfEdgesPerBucketTwo) noexcept {
		
			// Free number of edges per bucket two
			numberOfEdgesPerBucketTwo->release();
		});
		static const unique_ptr<MTL::Buffer, void(*)(MTL::Buffer *)> remainingEdgesOne(device->newBuffer(sizeof(uint32_t) + static_cast<uint64_t>(MAX_NUMBER_OF_EDGES_AFTER_TRIMMING) * EDGE_NUMBER_OF_COMPONENTS * sizeof(uint32_t), MTL::ResourceStorageModeShared | MTL::ResourceHazardTrackingModeUntracked), [](MTL::Buffer *remainingEdgesOne) noexcept {
		
			// Free remaining edges one
			remainingEdgesOne->release();
		});
		static const unique_ptr<MTL::Buffer, void(*)(MTL::Buffer *)> remainingEdgesTwo(device->newBuffer(sizeof(uint32_t) + static_cast<uint64_t>(MAX_NUMBER_OF_EDGES_AFTER_TRIMMING) * EDGE_NUMBER_OF_COMPONENTS * sizeof(uint32_t), MTL::ResourceStorageModeShared | MTL::ResourceHazardTrackingModeUntracked), [](MTL::Buffer *remainingEdgesTwo) noexcept {
...
// Otherwise
	...
	bool performMeanTrimmingLoop(const cl_context context) noexcept {
		...
		// Check if getting device's queue properties, name size, max work group size, max memory allocation size, or work group memory size failed, or its name or max work group size don't exists
		cl_command_queue_properties queueProperties;
		size_t nameSize;
		size_t maxWorkGroupSize;
		cl_ulong maxMemoryAllocationSize;
		cl_ulong workGroupMemorySize;
		if(clGetDeviceInfo(device, CL_DEVICE_QUEUE_PROPERTIES, sizeof(queueProperties), &queueProperties, nullptr) != CL_SUCCESS || clGetDeviceInfo(device, CL_DEVICE_NAME, 0, nullptr, &nameSize) != CL_SUCCESS || clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(maxWorkGroupSize), &maxWorkGroupSize, nullptr) != CL_SUCCESS || clGetDeviceInfo(device, CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(maxMemoryAllocationSize), &maxMemoryAllocationSize, nullptr) != CL_SUCCESS || clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(workGroupMemorySize), &workGroupMemorySize, nullptr) != CL_SUCCESS || !nameSize || !maxWorkGroupSize) {
		...
		if(clEnqueueNDRangeKernel(commandQueue.get(), stepTwoKernel.get(), 1, nullptr, &totalNumberOfWorkItems[1], &workItemsPerWorkGroup[1], 2, unmove((const cl_event []){runStepEvents[0].get(), clearNumberOfEdgesPerBucketEvents[1].get()}), runStepEvents[1].getAddress()) != CL_SUCCESS) {
		...
		if(TRIMMING_ROUNDS > 1) {
			...
			if(clEnqueueNDRangeKernel(commandQueue.get(), stepThreeKernel.get(), 1, nullptr, &totalNumberOfWorkItems[2], &workItemsPerWorkGroup[2], 1, clearNumberOfEdgesPerBucketEvents[2].getAddress(), runStepEvents[2].getAddress()) != CL_SUCCESS) {
			...
			if(TRIMMING_ROUNDS > 2) {
				...
				if(clEnqueueNDRangeKernel(commandQueue.get(), stepFourKernel.get(), 1, nullptr, &totalNumberOfWorkItems[3], &workItemsPerWorkGroup[3], 1, clearNumberOfEdgesPerBucketEvents[3].getAddress(), runStepEvents[3].getAddress()) != CL_SUCCESS) {
				...
				for(unsigned int i = 3; i < TRIMMING_ROUNDS; ++i) {
					...
					if(clEnqueueNDRangeKernel(commandQueue.get(), stepFiveKernel.get(), 1, nullptr, &totalNumberOfWorkItems[4], &workItemsPerWorkGroup[4], 1, clearNumberOfEdgesPerBucketEvents[i + 1].getAddress(), runStepEvents[i + 1].getAddress()) != CL_SUCCESS) {
		...
		if(clEnqueueFillBuffer(commandQueue.get(), remainingEdgesOne.get(), (const cl_uint[]){0}, sizeof(cl_uint), 0, sizeof(cl_uint), 0, nullptr, clearNumberOfEdgesPerBucketEvents[TRIMMING_ROUNDS + 1].getAddress()) != CL_SUCCESS) {
		...
		if(clEnqueueNDRangeKernel(commandQueue.get(), stepSixKernel.get(), 1, nullptr, &totalNumberOfWorkItems[5], &workItemsPerWorkGroup[5], 2, unmove((const cl_event []){runStepEvents[TRIMMING_ROUNDS].get(), clearNumberOfEdgesPerBucketEvents[TRIMMING_ROUNDS + 1].get()}), runStepEvents[TRIMMING_ROUNDS + 1].getAddress()) != CL_SUCCESS) {
		...
		static uint32_t *resultOne = reinterpret_cast<uint32_t *>(clEnqueueMapBuffer(commandQueue.get(), remainingEdgesOne.get(), CL_FALSE, CL_MAP_READ, 0, sizeof(cl_uint) + static_cast<uint64_t>(MAX_NUMBER_OF_EDGES_AFTER_TRIMMING) * EDGE_NUMBER_OF_COMPONENTS * sizeof(cl_uint), 1, runStepEvents[TRIMMING_ROUNDS + 1].getAddress(), mapEvent.getAddress(), nullptr));
		if(!resultOne) {
		...
		static const unique_ptr<uint32_t, void(*)(uint32_t *)> resultOneUniquePointer(resultOne, [](__attribute__((unused)) uint32_t *unused) noexcept {
			...
			if(resultOne) {
				...
				Event unmapEvent;
				clEnqueueUnmapMemObject(commandQueue.get(), remainingEdgesOne.get(), reinterpret_cast<void *>(resultOne), mapEvent.get() ? 1 : 0, mapEvent.getAddress(), unmapEvent.getAddress());
			}
		});
		...
		if(clEnqueueFillBuffer(commandQueue.get(), remainingEdgesTwo.get(), (const cl_uint[]){0}, sizeof(cl_uint), 0, sizeof(cl_uint), 0, nullptr, clearNumberOfEdgesPerBucketEvents[TRIMMING_ROUNDS + 1].getAddress()) != CL_SUCCESS) {
		...
		// Check if queuing map result failed
		static uint32_t *resultTwo = reinterpret_cast<uint32_t *>(clEnqueueMapBuffer(commandQueue.get(), remainingEdgesTwo.get(), CL_FALSE, CL_MAP_READ, 0, sizeof(cl_uint) + static_cast<uint64_t>(MAX_NUMBER_OF_EDGES_AFTER_TRIMMING) * EDGE_NUMBER_OF_COMPONENTS * sizeof(cl_uint), 1, runStepEvents[TRIMMING_ROUNDS + 1].getAddress(), mapEvent.getAddress(), nullptr));
		if(!resultTwo) {
		...
		static const unique_ptr<uint32_t, void(*)(uint32_t *)> resultTwoUniquePointer(resultTwo, [](__attribute__((unused)) uint32_t *unused) noexcept {
			...
			if(resultTwo) {
				...
				Event unmapEvent;
				clEnqueueUnmapMemObject(commandQueue.get(), remainingEdgesTwo.get(), reinterpret_cast<void *>(resultTwo), mapEvent.get() ? 1 : 0, mapEvent.getAddress(), unmapEvent.getAddress());
			}
		});
		...
		if(clEnqueueUnmapMemObject(commandQueue.get(), remainingEdgesOne.get(), reinterpret_cast<void *>(resultOne), 0, nullptr, unmapEventOne.getAddress()) != CL_SUCCESS) {
		...
		while(!closing) {
			...
			// Check if queuing clearing the beginning of remaining edges one on the device failed
			if(clEnqueueFillBuffer(commandQueue.get(), remainingEdgesOne.get(), (const cl_uint[]){0}, sizeof(cl_uint), 0, sizeof(cl_uint), 1, unmapEventOne.getAddress(), clearNumberOfEdgesPerBucketEvents[TRIMMING_ROUNDS + 1].getAddress()) != CL_SUCCESS) {
			...
			// Check if queuing unmap result failed
			if(clEnqueueUnmapMemObject(commandQueue.get(), remainingEdgesTwo.get(), reinterpret_cast<void *>(resultTwo), 0, nullptr, unmapEventTwo.getAddress()) != CL_SUCCESS) {
			...
			// Check if queuing clearing the beginning of remaining edges two on the device failed
			if(clEnqueueFillBuffer(commandQueue.get(), remainingEdgesTwo.get(), (const cl_uint[]){0}, sizeof(cl_uint), 0, sizeof(cl_uint), 1, unmapEventTwo.getAddress(), clearNumberOfEdgesPerBucketEvents[TRIMMING_ROUNDS + 1].getAddress()) != CL_SUCCESS) {
			...
			// Check if queuing map result failed
			resultTwo = reinterpret_cast<uint32_t *>(clEnqueueMapBuffer(commandQueue.get(), remainingEdgesTwo.get(), CL_FALSE, CL_MAP_READ, 0, sizeof(cl_uint) + static_cast<uint64_t>(MAX_NUMBER_OF_EDGES_AFTER_TRIMMING) * EDGE_NUMBER_OF_COMPONENTS * sizeof(cl_uint), 1, runStepEvents[TRIMMING_ROUNDS + 1].getAddress(), mapEvent.getAddress(), nullptr));
			if(!resultTwo) {
