...
ifneq (,$(findstring android,$(CC)))
	...
	ifeq ($(OS),Windows_NT)
		...
		# Set flags and link libraries
		LIBS += -L"./opencl_loader/dist/$(shell for %%a in ("$(CC)") do for /f "tokens=1 delims=-" %%b in ('echo %%~na') do echo %%b)/lib" -lOpenCL -llog
