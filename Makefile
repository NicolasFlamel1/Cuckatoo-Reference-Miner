...
ifneq (,$(findstring android,$(CC)))
	...
	ifeq ($(OS),Windows_NT)
		...
		# Package command
		PACKAGE_COMMAND = && (if exist "./build" rd /q /s "./build" > "nul") && md "./build/apk/lib/$(shell type "./opencl_loader/dist/$(shell for %%a in ("$(CC)") do for /f "tokens=1 delims=-" %%b in ('echo %%~na') do echo %%b)\abi")" && move "./$(PROGRAM_NAME)" "./build/apk/lib/$(shell type "./opencl_loader/dist/$(shell for %%a in ("$(CC)") do for /f "tokens=1 delims=-" %%b in ('echo %%~na') do echo %%b)\abi")" > "nul" && "$(shell echo $(JBR_BIN))\javac" -classpath "$(shell echo $(ANDROID_JAR))" -d "./build" "./android/MainActivity.java" && set PATH="$(shell echo $(JBR_BIN))";%PATH% && "$(shell echo $(BUILD_TOOLS))\d8" --release --lib "$(shell echo $(ANDROID_JAR))" --output "./build/apk" "./build/com/$(subst $\ ,_,$(NAME))/MainActivity.class" && ((echo | set /p a="<?xml version="1.0" encoding="utf-8"?>") && echo.&& (echo | set /p a="<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.$(subst $\ ,_,$(NAME))" versionCode="$(VERSION)" versionName="$(VERSION)">") && echo.&& (echo | set /p a="<uses-sdk android:minSdkVersion="$(shell for %%a in ("$(CC)") do for /f "tokens=3 delims=-" %%b in ('echo %%~na') do for /f "tokens=3 delims=d" %%c in ('echo %%b') do echo %%c)"/>") && echo.&& (echo | set /p a="<application android:label="$(NAME) v$(VERSION)" android:hardwareAccelerated="false">") && echo.&& (echo | set /p a="<activity android:name=".MainActivity" android:configChanges=") && echo "colorMode|density|fontScale|fontWeightAdjustment|grammaticalGender|keyboard|keyboardHidden|layoutDirection|locale|mcc|mnc|navigation|orientation|screenLayout|screenSize|smallestScreenSize|touchscreen|uiMode"&& (echo | set /p a=">") && echo.&& (echo | set /p a="<intent-filter>") && echo.&& (echo | set /p a="<action android:name="android.intent.action.MAIN"/>") && echo.&& (echo | set /p a="<category android:name="android.intent.category.LAUNCHER"/>") && echo.&& (echo | set /p a="</intent-filter>") && echo.&& (echo | set /p a="</activity>") && echo.&& (echo | set /p a="</application>") && echo.&& (echo | set /p a="</manifest>") && echo.) > "./build/AndroidManifest.xml" && "$(shell echo $(BUILD_TOOLS))\aapt" package -M "./build/AndroidManifest.xml" -I "$(shell echo $(ANDROID_JAR))" -F "./build/$(NAME).apk" "./build/apk" && "$(shell echo $(BUILD_TOOLS))\zipalign" -p 4 "./build/$(NAME).apk" "./build/$(NAME)_aligned.apk" && "$(shell echo $(JBR_BIN))\keytool" -genkeypair -keystore "./build/keystore.jks" -keyalg RSA -storepass android -dname CN= > "nul" 2>&1 && "$(shell echo $(BUILD_TOOLS))\apksigner" sign --ks "./build/keystore.jks" --ks-pass pass:android --out "./$(NAME).apk" "./build/$(NAME)_aligned.apk" && rd /q /s "./build" > "nul"
	...
	# Otherwise
		...
		# Package command
		PACKAGE_COMMAND = && rm -rf "./build" && mkdir -p "./build/apk/lib/$(shell cat "./opencl_loader/dist/$(shell basename "$(CC)" | cut -d"-" -f1)/abi")" && mv "./$(PROGRAM_NAME)" "./build/apk/lib/$(shell cat "./opencl_loader/dist/$(shell basename "$(CC)" | cut -d"-" -f1)/abi")" && "$(shell echo $(JBR_BIN))/javac" -classpath "$(shell echo $(ANDROID_JAR))" -d "./build" "./android/MainActivity.java" && PATH="$(shell echo $(JBR_BIN))":$$PATH "$(shell echo $(BUILD_TOOLS))/d8" --release --lib "$(shell echo $(ANDROID_JAR))" --output "./build/apk" "./build/com/$(subst $\ ,_,$(NAME))/MainActivity.class" && echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<manifest xmlns:android=\"http://schemas.android.com/apk/res/android\" package=\"com.$(subst $\ ,_,$(NAME))\" versionCode=\"$(VERSION)\" versionName=\"$(VERSION)\">\n<uses-sdk android:minSdkVersion=\"$(shell basename "$(CC)" | cut -d"-" -f3 | cut -d"d" -f3)\"/>\n<application android:label=\"$(NAME) v$(VERSION)\" android:hardwareAccelerated=\"false\">\n<activity android:name=\".MainActivity\" android:configChanges=\"colorMode|density|fontScale|fontWeightAdjustment|grammaticalGender|keyboard|keyboardHidden|layoutDirection|locale|mcc|mnc|navigation|orientation|screenLayout|screenSize|smallestScreenSize|touchscreen|uiMode\"\n>\n<intent-filter>\n<action android:name=\"android.intent.action.MAIN\"/>\n<category android:name=\"android.intent.category.LAUNCHER\"/>\n</intent-filter>\n</activity>\n</application>\n</manifest>" > "./build/AndroidManifest.xml" && "$(shell echo $(BUILD_TOOLS))/aapt" package -M "./build/AndroidManifest.xml" -I "$(shell echo $(ANDROID_JAR))" -F "./build/$(NAME).apk" "./build/apk" && "$(shell echo $(BUILD_TOOLS))/zipalign" -p 4 "./build/$(NAME).apk" "./build/$(NAME)_aligned.apk" && "$(shell echo $(JBR_BIN))/keytool" -genkeypair -keystore "./build/keystore.jks" -keyalg RSA -storepass android -dname CN= > "/dev/null" 2>&1 && PATH="$(shell echo $(JBR_BIN))":$$PATH "$(shell echo $(BUILD_TOOLS))/apksigner" sign --ks "./build/keystore.jks" --ks-pass pass:android --out "./$(NAME).apk" "./build/$(NAME)_aligned.apk" && rm -r "./build"
	endif
