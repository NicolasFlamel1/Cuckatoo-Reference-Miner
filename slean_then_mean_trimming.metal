...
// Check is slean trimming parts is two
	...
	[[kernel]] void trimEdgesStepThirtyFive(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Otherwise
#else

	// Trim edges step thirty-five
	[[kernel]] void trimEdgesStepThirtyFive(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, device uint *nodesBitmap, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);
#endif

// Trim edges step thirty-six
[[kernel]] void trimEdgesStepThirtySix(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device const uint *nodesBitmap, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Check is slean trimming parts is four
#if SLEAN_TRIMMING_PARTS == 4

	// Trim edges step thirty-seven
	[[kernel]] void trimEdgesStepThirtySeven(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Otherwise
#else

	// Trim edges step thirty-seven
	[[kernel]] void trimEdgesStepThirtySeven(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, device uint *nodesBitmap, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);
#endif
...
// Check is slean trimming parts is eight
	...
	[[kernel]] void trimEdgesStepThirtyEight(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Otherwise
#else

	// Trim edges step thirty-eight
	[[kernel]] void trimEdgesStepThirtyEight(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, device uint *nodesBitmap, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);
#endif

// Trim edges step thirty-nine
[[kernel]] void trimEdgesStepThirtyNine(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step fourty
[[kernel]] void trimEdgesStepFourty(device const uint *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step fourty-one
[[kernel]] void trimEdgesStepFourtyOne(device const uint *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint2 *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step fourty-two
[[kernel]] void trimEdgesStepFourtyTwo(device const uint2 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint4 *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Trim edges step fourty-three
[[kernel]] void trimEdgesStepFourtyThree(device const uint4 *sourceBuckets, device const uint *numberOfEdgesPerSourceBucket, device uint4 *destinationBuckets, device atomic_uint *numberOfEdgesPerDestinationBucket, const ushort localId, const ushort localSize, const uint groupId);

// Check if mean trimming rounds is one
#if MEAN_TRIMMING_ROUNDS == 1

	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint *buckets, device const uint *numberOfEdgesPerBucket, device atomic_uint *remainingEdges, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Otherwise check if mean trimming rounds is two
#elif MEAN_TRIMMING_ROUNDS == 2

	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint2 *buckets, device const uint *numberOfEdgesPerBucket, device atomic_uint *remainingEdges, constant const ulong4 &sipHashKeys, const ushort localId, const ushort localSize, const uint groupId);

// Otherwise
#else

	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint4 *buckets, device const uint *numberOfEdgesPerBucket, device atomic_uint *remainingEdges, const ushort localId, const ushort localSize, const uint groupId);
#endif
...
// Check if number of mean buckets is one
#if NUMBER_OF_MEAN_BUCKETS == 1

	// Clear number of edges per mean bucket
	[[kernel]] void clearNumberOfEdgesPerMeanBucket(device uint *numberOfEdgesPerMeanBucket, const uint globalId);
	
// Otherwise
#else

	// Clear number of edges per mean bucket
	[[kernel]] void clearNumberOfEdgesPerMeanBucket(device ulong *numberOfEdgesPerMeanBucket, const uint globalId);
#endif
		...
		// Check if local buckets size isn't one
			...
			// Check if slean trimming rounds is one
				...
				if(isBitSetInBitmap(bitmap, (sipHash24(sipHashKeys, static_cast<ulong>(edgeIndex) * 2) & BITMAP_MASK) ^ 1)) {
					...
					const uint bucketIndex = sipHash24(sipHashKeys, (static_cast<ulong>(edgeIndex) * 2) | 1) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_MEAN_BUCKET_SORTING;
					
					// Get bucket's next edge index
					const uint nextEdgeIndex = min(atomic_fetch_add_explicit(&numberOfEdgesPerDestinationBucket[bucketIndex], 1, memory_order_relaxed), static_cast<uint>(INITIAL_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET - 1));
					
					// Get destination bucket's next indices
					device uint *bucketNextIndices = &destinationBuckets[INITIAL_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET * bucketIndex + nextEdgeIndex];
		...
		// Check if local buckets size isn't one
			...
			// Check if slean trimming rounds is two
				...
				if(isBitSetInBitmap(bitmap, (sipHash24(sipHashKeys, (static_cast<ulong>(edgeIndex) * 2) | 1) & BITMAP_MASK) ^ 1)) {
					...
					// Get edge's other node's bucket index
					const uint bucketIndex = sipHash24(sipHashKeys, static_cast<ulong>(edgeIndex) * 2) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_MEAN_BUCKET_SORTING;
...
// Check is slean trimming parts is two
	...
	// Trim edges step thirty-five
	[[kernel]] void trimEdgesStepThirtyFive(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

// Otherwise
#else

	// Trim edges step thirty-five
	[[kernel]] void trimEdgesStepThirtyFive(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], device uint *nodesBitmap [[buffer(5)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
#endif
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		if(isBitSetInBitmap(bitmap, edgeIndexAndNode.y ^ 1)) {
			...
			// Check if slean trimming rounds is even
				...
				// Get edge's other node's bucket index
				const uint bucketIndex = sipHash24(sipHashKeys, static_cast<ulong>(edgeIndexAndNode.x) * 2) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_MEAN_BUCKET_SORTING;
				
			// Otherwise
			#else
			
				// Get edge's other node's bucket index
				const uint bucketIndex = sipHash24(sipHashKeys, (static_cast<ulong>(edgeIndexAndNode.x) * 2) | 1) >> NUMBER_OF_LEAST_SIGNIFICANT_BITS_IGNORED_DURING_MEAN_BUCKET_SORTING;
			#endif
...
// Trim edges step thirty-six
[[kernel]] void trimEdgesStepThirtySix(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device const uint *nodesBitmap [[buffer(5)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
...
// Check is slean trimming parts is four
	...
	// Trim edges step thirty-seven
	[[kernel]] void trimEdgesStepThirtySeven(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

// Otherwise
#else

	// Trim edges step thirty-seven
	[[kernel]] void trimEdgesStepThirtySeven(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], device uint *nodesBitmap [[buffer(5)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
#endif
...
// Check is slean trimming parts is eight
	...
	// Trim edges step thirty-eight
	[[kernel]] void trimEdgesStepThirtyEight(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

// Otherwise
#else

	// Trim edges step thirty-eight
	[[kernel]] void trimEdgesStepThirtyEight(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], device uint *nodesBitmap [[buffer(5)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
#endif
...
// Trim edges step thirty-nine
[[kernel]] void trimEdgesStepThirtyNine(device const uint2 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
...
// Trim edges step fourty
[[kernel]] void trimEdgesStepFourty(device const uint *sourceBuckets [[buffer(6)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(7)]], device uint *destinationBuckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(1)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

	// Declare bitmap
	threadgroup atomic_uint bitmap[static_cast<short>(NUMBER_OF_MEAN_BITMAP_BYTES / sizeof(uint))];
	
	// Go through all groups of bytes in the bitmap as a work group
	for(short i = localId; i < static_cast<short>(NUMBER_OF_MEAN_BITMAP_BYTES / sizeof(uint)); i += localSize) {
	...
	// Get number of edges in this work group's bucket
	const uint numberOfEdges = min(numberOfEdgesPerSourceBucket[groupId], static_cast<uint>(INITIAL_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET));
	
	// Get work group's bucket's indices
	device const uint *indices = &sourceBuckets[INITIAL_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET * groupId];
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		// Check if slean trimming rounds is even
			...
			// Enable edge's node in the bitmap
			setBitInBitmap(bitmap, sipHash24(sipHashKeys, static_cast<ulong>(edgeIndex) * 2) & MEAN_BITMAP_MASK);
			
		// Otherwise
		#else
		
			// Enable edge's node in the bitmap
			setBitInBitmap(bitmap, sipHash24(sipHashKeys, (static_cast<ulong>(edgeIndex) * 2) | 1) & MEAN_BITMAP_MASK);
		#endif
	}
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		// Check if slean trimming rounds is even
			...
			// Check if edge's node has a pair in the bitmap
			if(isBitSetInBitmap(bitmap, (sipHash24(sipHashKeys, static_cast<ulong>(edgeIndex) * 2) & MEAN_BITMAP_MASK) ^ 1)) {
		...
		// Otherwise
			...
			// Check if edge's node has a pair in the bitmap
			if(isBitSetInBitmap(bitmap, (sipHash24(sipHashKeys, (static_cast<ulong>(edgeIndex) * 2) | 1) & MEAN_BITMAP_MASK) ^ 1)) {
			...
			// Get bucket's next edge index
			const uint nextEdgeIndex = min(atomic_fetch_add_explicit(&numberOfEdgesPerDestinationBucket[bucketIndex], 1, memory_order_relaxed), static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET - 1));
			
			// Get destination bucket's next indices
			device uint *bucketNextIndices = &destinationBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET * bucketIndex + nextEdgeIndex];
...
// Trim edges step fourty-one
[[kernel]] void trimEdgesStepFourtyOne(device const uint *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint2 *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	// Get number of edges in this work group's bucket
	const uint numberOfEdges = min(numberOfEdgesPerSourceBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET));
	
	// Get work group's bucket's indices
	device const uint *indices = &sourceBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET * groupId];
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		// Otherwise
			...
			// Get bucket's next edge index
			const uint nextEdgeIndex = min(atomic_fetch_add_explicit(&numberOfEdgesPerDestinationBucket[bucketIndex], 1, memory_order_relaxed), static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2 - 1));
			
			// Get destination bucket's next indices
			device uint2 *bucketNextIndices = &destinationBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2 * bucketIndex + nextEdgeIndex];
...
// Trim edges step fourty-two
[[kernel]] void trimEdgesStepFourtyTwo(device const uint2 *sourceBuckets [[buffer(6)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(7)]], device uint4 *destinationBuckets [[buffer(0)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(1)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	// Get number of edges in this work group's bucket
	const uint numberOfEdges = min(numberOfEdgesPerSourceBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2));
	
	// Get work group's bucket's indices
	device const uint2 *indices = &sourceBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2 * groupId];
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		if(isBitSetInBitmap(bitmap, (edgeIndexAndNode.y & MEAN_BITMAP_MASK) ^ 1)) {
			...
			// Otherwise
				...
				// Get edge's other node
				const uint otherNode = sipHash24(sipHashKeys, static_cast<ulong>(edgeIndexAndNode.x) * 2);
			#endif
			...
			// Get bucket's next edge index
			const uint nextEdgeIndex = min(atomic_fetch_add_explicit(&numberOfEdgesPerDestinationBucket[bucketIndex], 1, memory_order_relaxed), static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4 - 1));
			
			// Get destination bucket's next indices
			device uint4 *bucketNextIndices = &destinationBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4 * bucketIndex + nextEdgeIndex];
...
// Trim edges step fourty-three
[[kernel]] void trimEdgesStepFourtyThree(device const uint4 *sourceBuckets [[buffer(0)]], device const uint *numberOfEdgesPerSourceBucket [[buffer(1)]], device uint4 *destinationBuckets [[buffer(6)]], device atomic_uint *numberOfEdgesPerDestinationBucket [[buffer(7)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
	...
	// Get number of edges in this work group's bucket
	const uint numberOfEdges = min(numberOfEdgesPerSourceBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4));
	
	// Get work group's bucket's indices
	device const uint4 *indices = &sourceBuckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4 * groupId];
...
// Check if mean trimming rounds is one
	...
	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint *buckets [[buffer(6)]], device const uint *numberOfEdgesPerBucket [[buffer(7)]], device atomic_uint *remainingEdges [[buffer(10)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

// Otherwise check if mean trimming rounds is two
#elif MEAN_TRIMMING_ROUNDS == 2

	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint2 *buckets [[buffer(6)]], device const uint *numberOfEdgesPerBucket [[buffer(7)]], device atomic_uint *remainingEdges [[buffer(10)]], constant const ulong4 &sipHashKeys [[buffer(2)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {

// Otherwise
#else

	// Trim edges step fourty-four
	[[kernel]] void trimEdgesStepFourtyFour(device const uint4 *buckets [[buffer(6)]], device const uint *numberOfEdgesPerBucket [[buffer(7)]], device atomic_uint *remainingEdges [[buffer(10)]], const ushort localId [[thread_position_in_threadgroup]], const ushort localSize [[threads_per_threadgroup]], const uint groupId [[threadgroup_position_in_grid]]) {
#endif
	...
	// Check if mean trimming rounds is one
		...
		// Get number of edges in this work group's bucket
		const uint numberOfEdges = min(numberOfEdgesPerBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET));
		
		// Get work group's bucket's indices
		device const uint *indices = &buckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET * groupId];
	
	// Otherwise check if mean trimming rounds is two
	#elif MEAN_TRIMMING_ROUNDS == 2
	
		// Get number of edges in this work group's bucket
		const uint numberOfEdges = min(numberOfEdgesPerBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2));
		
		// Get work group's bucket's indices
		device const uint2 *indices = &buckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 2 * groupId];
		
	// Otherwise
	#else
	
		// Get number of edges in this work group's bucket
		const uint numberOfEdges = min(numberOfEdgesPerBucket[groupId], static_cast<uint>(AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4));
		
		// Get work group's bucket's indices
		device const uint4 *indices = &buckets[AFTER_TRIMMING_ROUND_MAX_NUMBER_OF_EDGES_PER_MEAN_BUCKET / 4 * groupId];
	#endif
	...
	for(uint i = localId; i < numberOfEdges; i += localSize) {
		...
		// Otherwise check if mean trimming rounds is two
			...
			// Otherwise
				...
				// Get edge's other node
				const uint otherNode = sipHash24(sipHashKeys, static_cast<ulong>(indices[i].x) * 2);
				
				// Set next remaining edge to the edge and its nodes
				atomic_store_explicit(&nextRemainingEdge[0], indices[i].x, memory_order_relaxed);
				atomic_store_explicit(&nextRemainingEdge[1], otherNode, memory_order_relaxed);
...
// Check if number of mean buckets is one
	...
	// Clear number of edges per mean bucket
	[[kernel]] void clearNumberOfEdgesPerMeanBucket(device uint *numberOfEdgesPerMeanBucket [[buffer(7)]], const uint globalId [[thread_position_in_grid]]) {
	
// Otherwise
#else

	// Clear number of edges per mean bucket
	[[kernel]] void clearNumberOfEdgesPerMeanBucket(device ulong *numberOfEdgesPerMeanBucket [[buffer(7)]], const uint globalId [[thread_position_in_grid]]) {
#endif

	// Set this work item's value in number of edges per mean bucket to zero
	numberOfEdgesPerMeanBucket[globalId] = 0;
}
